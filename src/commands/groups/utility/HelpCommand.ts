...
export class HelpCommand extends Command {
  ...
  data = new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('command').setDescription('Specific command to get help for'),
    );
  ...
  private async showAllCommands(interaction: CommandContext['interaction']): Promise<void> {
    const { commandRegistry } = await import('../../../services/index.js');
  ...
  private async showCommandHelp(interaction: CommandContext['interaction'], name: string): Promise<void> {
    const { commandRegistry } = await import('../../../services/index.js');
