...
export async function loadCommands(): Promise<void> {
  const { PingCommand, HelpCommand } = await import('./commands/groups/utility');
  const { BanCommand, KickCommand, MuteCommand, WarnCommand, PurgeCommand } = await import('./commands/groups/moderation');
